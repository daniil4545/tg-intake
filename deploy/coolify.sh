...
Секреты берутся из окружения, иначе из Keychain:
  COOLIFY_TOKEN          <- служба intake-coolify-token
  COOLIFY_API_BASE       <- служба intake-coolify-api
...
token="$(need COOLIFY_TOKEN intake-coolify-token)"
api_base="$(need COOLIFY_API_BASE intake-coolify-api)"
