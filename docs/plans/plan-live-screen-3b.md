...
| 15 | сбор: два материала, новый `Bot`, третий | один `sendMessage`, дальше правка того же id | `TestTallyIsScreen` |
...
| 20 | S3: «Публикую», «Всё так», «Поправить» без активного обращения; `Active` вернул ошибку | `answerCallbackQuery` на каждое нажатие | `TestStepButtonAlwaysAnswers` |
