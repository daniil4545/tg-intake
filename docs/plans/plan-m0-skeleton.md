...
- Таблицы `cases`, `case_items`, `case_events`, `jobs` заведены, но не
  используются до M1.
- Закрепление `api.telegram.org` за рабочим DC: подтвердилось на первом же
  выкате. Дефолтный IP `149.154.166.110` с этого VPS не отвечает, `getMe`
  уходит в таймаут и приложение перезапускается по кругу. В dev-Compose стоит
  `extra_hosts` на `149.154.167.220`, как у соседнего Bot API сервиса.
- Healthcheck при этом показывал `healthy`: он пингует только базу. Это
  зафиксированное ограничение среза, живость поллера появится в M1 вместе с
