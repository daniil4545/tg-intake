...
Строка 19 идёт через маршрут: регистрация хендлеров вынесена из `NewBot` в `routes(tb)` (её
поднимает поллер), тест зовёт `b.routes(tb)` и кормит апдейт `tb.ProcessUpdate` - так тест
ловит и потерю привязки `tele.OnCallback` к `staleButton`, а не только сам `staleButton`.
