...
func TestCardFitsOneMessage(t *testing.T) {
	...
	// Тикет, заведённый до появления краткого содержания: показывается первый
	// раздел саммари целиком, а следующий уже нет - иначе в карточку попадал
	// хвост одного вопроса и обрывок другого.
	ticket.Brief = ""
	ticket.Body = "## Какую задачу это решает\n\nОтказы теряются.\n\n## Как справляются сейчас\n\nВедут вручную."
	text = cardText(&ticket)
	if !strings.Contains(text, "Отказы теряются") {
		t.Errorf("у тикета без краткого содержания карточка молчит о сути:\n%s", text)
	}
	if strings.Contains(text, "Ведут вручную") {
		t.Errorf("в карточку попал второй раздел:\n%s", text)
	}
	if !strings.Contains(text, "Какую задачу это решает") {
		t.Errorf("раздел показан без названия:\n%s", text)
	}
}
