...
// тикет. Остальное (ФИО, куски переписки) - забота промтов: надёжный детектор
// это отдельный проект, а последняя защита у нас есть - автор видит саммари до
// публикации.
//
// Шаблоны намеренно узкие. Широкий шаблон телефона съел бы номера заказов и
// идентификаторы карточек, без которых тикет бесполезен.
var (
