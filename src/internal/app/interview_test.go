...
func TestContractHoldsReadiness(t *testing.T) {
	...
	// Путь до готовности стоит автору вопросов, а его длину задаёт число
	// обязательных пунктов. Правка правил не имеет права удлинить разговор молча:
	// раундов три, и каждый новый обязательный пункт приближает метку неполноты.
	required := 0
	...
	if required != 4 {
		t.Errorf("обязательных пунктов у пожелания: %d, ожидалось 4", required)
	}
}
